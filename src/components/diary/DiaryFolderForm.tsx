...
import { diaryFolderSchema, DiaryFolderFormValues } from "@/lib/validations";

interface DiaryFolderFormProps {
...
export function DiaryFolderForm({ action, folders, currentParentId, cancelHref }: DiaryFolderFormProps) {
  ...
  } = useForm<DiaryFolderFormValues>({
    resolver: zodResolver(diaryFolderSchema),
    defaultValues: {
      name: "",
      description: "",
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit(() => action(new FormData(formRef.current!)))} className="space-y-6">
      <div>
        ...
        <input {...register("name")} placeholder="Ex: Capítulo 1" className={inputClass} />
